...
-(void)viewWillAppear:(BOOL)animated {
    ...
    for (int i = 0; i<[issueSorter count]; i++ ) {
        if ([[[issueSorter objectAtIndex:i] objectForKey:@"status"] isEqual: @"open"] && [[[issueSorter objectAtIndex:i] objectForKey:@"signaturesNeeded"] integerValue] >0 ) {
        ...
        } else if  ([[[issueSorter objectAtIndex:i] objectForKey:@"status"] isEqual: @"pending response"] && [[[issueSorter objectAtIndex:i] objectForKey:@"signaturesNeeded"] integerValue] ==0 ){
...
//    [cell.contentView addSubview:favoriteSwitch];
    ...
    NSString *signaturesNeededText = [NSString stringWithFormat:@"Signatures Count: %@", [[[peitionTableViewArray objectAtIndex: indexPath.section] objectAtIndex:indexPath.row] objectForKey:@"signatureCount"] ];
